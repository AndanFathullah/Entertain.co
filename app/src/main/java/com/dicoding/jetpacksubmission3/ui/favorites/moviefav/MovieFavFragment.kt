...
class MovieFavFragment : Fragment() {
    ...
    private lateinit var moviesTrendAdapter: MovieTrendFavAdapter
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        itemTouchHelperTrend.attachToRecyclerView(fragmentMovieFavBinding.rvMovieTrendFav)
        ...
        moviesTrendAdapter = MovieTrendFavAdapter()
        viewModel.getMoviesTrendFav().observe(viewLifecycleOwner, {movieTrendFav ->
            loading(false)
            moviesTrendAdapter.submitList(movieTrendFav)
        })

        with(fragmentMovieFavBinding.rvMovieTrendFav){
            layoutManager = LinearLayoutManager(context)
            adapter = moviesTrendAdapter
        }
    }
    ...
    private val itemTouchHelperTrend = ItemTouchHelper(object : ItemTouchHelper.Callback() {
        ...
        override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
            if (view != null) {
                val swipedPosition = viewHolder.adapterPosition
                val courseEntity = moviesTrendAdapter.getSwipedData(swipedPosition)
                courseEntity?.let { viewModel.setMoviesTrendFav(it) }
                val snackbar = Snackbar.make(view as View, R.string.message_undo, Snackbar.LENGTH_LONG)
                snackbar.setAction(R.string.message_ok) { v ->
                    courseEntity?.let { viewModel.setMoviesTrendFav(it) }
                }
